...
func MoveCompletedTask(id int) error {
	...
	originalID, description, due_date, start_date, finish_date, status, notes, err := SelectData(id)
	if err != nil {
		return err
	}
	...
	_, err = stmt.Exec(originalID, description, due_date, start_date, finish_date, status, notes)
	if err != nil {
		return err
	}
